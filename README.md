...
| | |
|---|---|
| geolonia 出力との一致 | **99.46%** |
| 町字以上に到達 | **99.7%** |
| ファストパス（Jev 不要） | 99.5% |
| Jev 往復 | 15 回（7,191 件全体で） |
| コスト | **$0.0021**（$0.0003 / 1,000 件） |
| 1 往復の時間 | p50 227ms |
| 全体の処理時間 | 5.1 秒（0.71 ms/件） |
| 索引 | `town.marisa` 24MB + `abr.db` 61MB（町字まで） |

一致率は「geolonia との合意率」であって正解率ではない（[docs/eval.md](docs/eval.md) §4）。

異体字は Jev が解く。`箪笥町`→`簞笥町` (0.97)、`不動ヶ岡`→`不動ケ岡` (0.96)、
`三栄町`→`四谷三栄町` (0.83)。編集距離では届かなかった精度で、
**絞り込みを諦めて判断を委ねるほうが強い**という設計判断の裏付けになっている。
